...
function App() {
  ...
  return (
    <div className="min-h-screen bg-zinc-50 flex">
      {/* Event Panel Sidebar */}
      {showEventPanel && (
        <aside className="w-80 border-r border-zinc-200 p-4 bg-zinc-100">
          <EventPanel sseConnected={daemonStatus.status === 'connected'} />
        </aside>
      )}
          ...
          <div className="flex items-center justify-between max-w-7xl mx-auto">
            <div className="flex items-center gap-4">
              ...
              <button 
                ...
                className={`px-2 py-1 text-xs rounded border ${showEventPanel ? 'border-blue-500 bg-blue-100 text-blue-700' : 'border-zinc-200 bg-white text-zinc-500'}`}
              >
                Events {showEventPanel ? '✓' : ''}
              </button>
            </div>
            <div className="flex items-center gap-4">
